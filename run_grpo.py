...
def compute_advantages_grpo(rewards, group_size=4):
    ...
    grouped_advantages = (grouped_rewards - group_mean) / group_std
...
def train_grpo(
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, batch in enumerate(progress_bar):
            ...
            # ====================================================================
            gen_data = generate_completions(model, tokenizer, input_ids, attention_mask, max_new_tokens, num_samples=group_size)
            seq_ids, seq_mask = gen_data["output_ids"], (gen_data["output_ids"] != tokenizer.pad_token_id).long()
            rewards = compute_reward(gen_data["completions"], [a for a in answers for _ in range(group_size)]).to(device)
            advantages = compute_advantages_grpo(rewards, group_size).to(device)
            logprobs = compute_logprobs_from_model(model, seq_ids, seq_mask)
            loss_mask = seq_mask.clone(); loss_mask[:, :gen_data["prompt_length"]] = 0
            loss = compute_policy_loss(logprobs, logprobs.detach(), advantages, loss_mask, clip_eps)
